/////////////////////////////////////
// Komorebi Shader Shaders by EminGT //
/////////////////////////////////////
...
#include "/lib/komorebi/komorebi_tonemap.glsl"

void DoKomorebiTonemap(inout vec3 color) {
    color = komorebi_tonemapWithTM(color, TM_EXPOSURE);
    LinearToRGB(color);
}

#if 0
void DoCompTonemap(inout vec3 color) {
    // Lottes tonemap modified for Komorebi Shader Shaders
...
void DoBSLColorSaturation(inout vec3 color) {
    ...
    void DoBloom(inout vec3 color, vec2 coord, float dither, float lViewPos) {
        ...
        blur = mix(blur, sqrt(max(blur, vec3(0.0))), 0.22 * KOMOREBI_BLOOM_DIFFUSION);
        blur *= mix(vec3(1.0), vec3(1.03, 1.00, 0.97), 0.22 * KOMOREBI_SUN_WARMTH);

        float bloomStrength = (BLOOM_STRENGTH + 0.2 * darknessFactor) * KOMOREBI_BLOOM_SCALE;
        float colorLum = GetLuminance(max(color, vec3(0.0)));
        float bloomMask = smoothstep(0.28, 1.2, colorLum);
        float distanceMask = smoothstep(0.0, 1.0, lViewPos / max(renderDistance, 80.0));
        bloomStrength *= mix(0.75, 1.0, bloomMask);
        bloomStrength *= mix(0.85, 1.05, distanceMask * KOMOREBI_BLOOM_DIFFUSION * 0.45);
...
void main() {
    ...
    // Komorebi: colour response is handled in tonemap + final post-grade (skip BSL saturation pass).
