    ...
    vec3 GetSky(float VdotU, float VdotS, float dither, bool doGlare, bool doGround) {
        ...
        // Komorebi style: add warm horizon haze and slightly softer gradient contrast.
        float horizonFactor = smoothstep(0.0, 1.0, 1.0 - VdotUmax0);
        vec3 hazeColor = mix(dayDownSkyColor, sunsetDownSkyColorP, 0.45 * invNoonFactor + 0.15 * sunFactor);
        hazeColor += vec3(0.05, 0.02, -0.01) * KOMOREBI_FOG_WARMTH * (0.35 + 0.65 * sunFactor);
        finalSky = mix(finalSky, hazeColor, KOMOREBI_ATMOS_HAZE * horizonFactor * invRainFactor * 0.55);
        finalSky = mix(finalSky, sqrt(max(finalSky, vec3(0.0))), KOMOREBI_SKY_PASTEL * 0.18);
        ...
        if (doGlare) {
            if (0.0 < VdotSML) {
                ...
                vec3 glareColor = mix(vec3(0.38, 0.4, 0.5) * 0.3, vec3(1.5, 0.7, 0.3) + vec3(0.0, 0.5, 0.5) * noonFactor, sunVisibility);
                     ...
                     glareColor = mix(glareColor, vec3(1.35, 0.98, 0.84), 0.18 * KOMOREBI_SUN_WARMTH * sunVisibility);
