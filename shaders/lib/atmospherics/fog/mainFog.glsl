#include "/lib/komorebi/komorebi_settings.glsl"
    ...
    void DoBorderFog(inout vec4 color, inout float skyFade, float lPos, float VdotU, float VdotS, float dither) {
        ...
        if (fog > 0.0) {
            ...
            #ifdef OVERWORLD
                fogColorM += vec3(0.045, 0.018, -0.01) * KOMOREBI_FOG_WARMTH * (0.35 + 0.65 * sunVisibility2);
            #endif
    ...
    void DoAtmosphericFog(inout vec4 color, vec3 playerPos, float lViewPos, float VdotS) {
            ...
            float fog = 1.0 - exp(-pow(lViewPos * (0.00092 - 0.00062 * rainFactor), 1.85 - 0.65 * rainFactor2) * lViewPos * renDisFactor);
        #else
            float fog = pow2(1.0 - exp(-max0(lViewPos - 36.0) * (0.78 + 0.62 * rainFactor) / ATM_FOG_DISTANCE));
        #endif
        
        fog *= (ATM_FOG_MULT - 0.1 - 0.15 * invRainFactor) * KOMOREBI_FOG_DENSITY;
        ...
        fog *= altitudeFactor * (0.92 + 0.08 * KOMOREBI_FOG_DENSITY);
        ...
        if (fog > 0.0) {
            ...
            #ifdef OVERWORLD
                vec3 warmFogTint = vec3(0.05, 0.02, -0.012) * KOMOREBI_FOG_WARMTH * (0.35 + 0.65 * sunVisibility2);
                float fogLum = dot(fogColorM, vec3(0.2126, 0.7152, 0.0722));
                fogColorM += warmFogTint;
                fogColorM = mix(vec3(fogLum), fogColorM, KOMOREBI_SKY_SATURATION);
                fogColorM = mix(fogColorM, sqrt(max(fogColorM, vec3(0.0))), 0.22 * KOMOREBI_ATMOS_HAZE);
            #endif
