...
void DoLighting(inout vec4 color, inout vec3 shadowMult, vec3 playerPos, vec3 viewPos, float lViewPos, vec3 geoNormal, vec3 normalM, float dither,
    ...
    #ifdef OVERWORLD
        // Komorebi style: avoid harsh dark cuts in cast shadows.
        float komShadow = clamp(KOMOREBI_SHADOW_SOFTNESS, 0.0, 1.0);
        vec3 liftedShadowColor = vec3(0.72, 0.70, 0.68) * (0.85 + 0.3 * sunVisibility2);
        shadowLightMult = mix(shadowLightMult, liftedShadowColor, komShadow * (0.20 + 0.35 * (1.0 - shadowMultFloat)));
        shadowMultFloat = min1(GetLuminance(shadowLightMult));
    #endif
    ...
    #ifdef OVERWORLD
        float komSoftDiffuse = clamp(KOMOREBI_SHADOW_SOFTNESS, 0.0, 1.0);
        finalDiffuse = mix(finalDiffuse, pow(max(finalDiffuse, vec3(0.0)), vec3(0.90)), komSoftDiffuse * 0.35);
    #endif
