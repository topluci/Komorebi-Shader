    ...
    #include "/lib/komorebi/komorebi_settings.glsl"
        ...
        // Komorebi style: pastel sky response and warmer horizon / sunset blend.
        vec3 horizonWarmTint = vec3(0.14, 0.06, -0.03) * KOMOREBI_SKY_HORIZON_WARMTH;
        dayDownSkyColor += horizonWarmTint * (0.45 + 0.55 * sunFactor);
        dayMiddleSkyColor += horizonWarmTint * (0.20 + 0.45 * invNoonFactor);
        sunsetDownSkyColorP = mix(sunsetDownSkyColorP, vec3(1.24, 0.76, 0.67), 0.28 * KOMOREBI_SKY_PASTEL);
        sunsetMiddleSkyColor += vec3(0.10, -0.01, 0.13) * (0.30 * KOMOREBI_SKY_PASTEL);
        sunsetDownSkyColor = sunsetDownSkyColorP * 0.5 + 0.25 * sunsetMiddleSkyColor;

        float dayUpLum = dot(dayUpSkyColor, vec3(0.2126, 0.7152, 0.0722));
        float dayMidLum = dot(dayMiddleSkyColor, vec3(0.2126, 0.7152, 0.0722));
        float dayDownLum = dot(dayDownSkyColor, vec3(0.2126, 0.7152, 0.0722));
        dayUpSkyColor = mix(vec3(dayUpLum), dayUpSkyColor, KOMOREBI_SKY_SATURATION);
        dayMiddleSkyColor = mix(vec3(dayMidLum), dayMiddleSkyColor, KOMOREBI_SKY_SATURATION);
        dayDownSkyColor = mix(vec3(dayDownLum), dayDownSkyColor, KOMOREBI_SKY_SATURATION);

        dayUpSkyColor = mix(dayUpSkyColor, sqrt(max(dayUpSkyColor, vec3(0.0))), 0.20 * KOMOREBI_SKY_PASTEL);
        dayMiddleSkyColor = mix(dayMiddleSkyColor, sqrt(max(dayMiddleSkyColor, vec3(0.0))), 0.25 * KOMOREBI_SKY_PASTEL);
        dayDownSkyColor = mix(dayDownSkyColor, sqrt(max(dayDownSkyColor, vec3(0.0))), 0.35 * KOMOREBI_SKY_PASTEL);
    #endif
